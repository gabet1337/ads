...
\begin{abstract}
In this project, we compare the practical performance for \textit{Fibonacci Heaps} and \textit{Binary Heaps}. The implementations of the Heaps are done in C++ and supports the operations \texttt{MakeHeap}, \texttt{FindMin}, \texttt{Insert}, \texttt{DeleteMin} and \texttt{DecreaseKey}. We argue about the worst-case time bounds for each of the operations and design and perform experiments where we measure the running times and number of comparisons of the operations for both priority queue implementations. Continuing our studies, we implement Dijkstra's algorithms for the single-source shortest path problem for weighted graphs with non-negative edge-weights, such that it can switch between the two priority queue implementations. Finally we describe families of graphs where Dijkstra's algorithm performs many respectively few \texttt{DecreaseKey} operations and perform experiments focussing on how the amortized $\mathcal{O}(1)$ \texttt{DecreaseKey} operation performs in our experimental setup.
\\
\\
\\
...
\section{Conclusion}
%TODO discuss that Fibonacci is worse on the average random cases but better in the worst case!
